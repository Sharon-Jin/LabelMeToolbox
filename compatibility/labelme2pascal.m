function labelme2pascal(D, databasename, HOMELMIMAGES, folderdestination, TraindataPercentage, NEWIMAGESIZE)
%
...
for n = 1:Nimages
    ...
    if exist(fullfile(folderdestination, databasename, 'JPEGImages', filename), 'file')
        ...
        img = LMimread(D, n, HOMELMIMAGES); % Load image
         ...
         % Scaling the image!!!
        [newannotation, newimg, crop, scaling, err, msg] = LMcookimage(D(n).annotation, img, 'maximagesize', NEWIMAGESIZE, 'impad', 0);
LMplot(newannotation, newimg);
        img = newimg;
        D(n).annotation = newannotation;
